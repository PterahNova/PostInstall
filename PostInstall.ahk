...
IniRead, LatestVersion-URL, config.ini, config, LatestVersion-URL
IniRead, ININame, config.ini, config, LatestVersion-INI
IniRead, Updater, config.ini, config, Updater

^+U::
update:
IniRead, CurVer, config.ini, config, Version
UrlDownloadToFile, %LatestVersion-URL%
IniRead, NextVer, %LatestVersion-INI%, config, Version
if (NextVer > CurVer) {
	FileDelete, %Updater%
...
IniRead, On, config.ini, config, On-Button
IniRead, Off, config.ini, config, Off-Button
IniRead, Button, config.ini, config, Empty-Button
IniRead, ahk, config.ini, config, AutoHotKey
IniRead, TitleL, config.ini, config, Title-Left
IniRead, TitleR, config.ini, config, Title-Right
IniRead, Background, config.ini, config, Background
IniRead, Minimize, config.ini, config, Minimize
IniRead, Close, config.ini, config, Close
...
Menu, Tray, Icon , PostInstall.ico
...
if (Opt10=1) {
	...
	WinWaitActive, Internet Security
		...
		sleep 500
			...
			Click, %X%, %Y%
			clipboard=%OrderNumber%@3000RPMRepairs.co.uk
			Send %clipboard%
			sleep 250
			}
